...
export function ProgressPage() {
  ...
  const selectedDay = month.find(day => day.date === selected);
  const milestones = [
  ...
  return <div className="mx-auto flex max-w-6xl flex-col gap-5 animate-fade-in">
    ...
    <section className={card + " sw-holo relative overflow-hidden"}>
      <div className="grid items-center gap-6 md:grid-cols-[1.25fr_1fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-extrabold tracking-tight sm:text-4xl">{activeDays ? `${activeDays} days of effort.` : "Your progress starts here."}</h2>
          <p className="mt-3 text-sm leading-6 text-white/60">{activeDays ? "Tap a day to see your progress." : "Check off a task, habit, or exercise to get started."}</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-7 gap-2">{lastWeek.map(day => {
        ...
        return <button type="button" key={day.date} aria-pressed={selected === day.date} aria-label={`View ${shortDate(day.date)} progress`} onClick={() => { setRange("daily"); setSelected(day.date); setShowAll(false); }} title={`${day.date}: ${day.tasksDone} tasks, ${day.exercisesDone} exercises, ${day.habitsDone} habits`} className={"rounded-xl border py-3 text-center " + (selected === day.date ? "border-[rgb(var(--sw-accent-rgb))] bg-[rgb(var(--sw-accent-rgb)/.2)]" : active ? "border-[rgb(var(--sw-accent-rgb)/.3)] bg-[rgb(var(--sw-accent-rgb)/.1)]" : "border-white/10 bg-white/[.02]")}>
      ...
      {range === "daily" && selectedDay && <div aria-live="polite" className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-sm font-semibold">{shortDate(selectedDay.date)}</p><div className="mt-3 grid grid-cols-3 gap-2">{(Object.keys(labels) as ProgressMetric[]).map(key => <div key={key}><p className="text-xs text-white/60">{labels[key]}</p><p className="mt-1 text-lg font-bold text-[rgb(var(--sw-accent-rgb))]">{pct(selectedDay[key])}</p></div>)}</div></div>}
    </section>
    ...
    <section className={card}>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div><h2 className="text-xl font-bold">Daily progress</h2></div>
      ...
      <p className="mt-3 text-sm leading-5 text-white/60" aria-live="polite">{selectedPoint ? `${shortDate(selectedPoint.key)}: ${pct(selectedPoint.value)}${range === "daily" ? "" : ` · ${selectedPoint.recorded} days recorded`}` : "Tap a bar to explore. — means no record, not 0%."}</p>
    ...
    <section className={card}>
      <div className="flex items-center gap-3"><Icon name="workout" className="h-6 w-6 text-[rgb(var(--sw-accent-rgb))]" /><h2 className="text-xl font-bold">Your latest best sets</h2></div>
      ...
      })}</div> : <p className="mt-4 text-sm leading-6 text-white/60">Log a best set in your workout to see how you improve over time.</p>}
      <details className="mt-4 text-xs leading-6 text-white/50"><summary className="cursor-pointer">About these comparisons</summary><p>Shows up to 6 exercises from the latest 120 workout records within 90 days. Use the same exercise variation and load convention when comparing.</p></details>
    </section>



    <section className={card}>
      <h2 className="text-xl font-bold">Your recorded days</h2>
    ...
    <details className={card}>
      <summary className="min-h-11 cursor-pointer text-base font-semibold">View details · Weekly comparison</summary>
    ...
    <details className={card}>
      <summary className="min-h-11 cursor-pointer text-base font-semibold">View details · Milestones ({earned.length} earned)</summary>
