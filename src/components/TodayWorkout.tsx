...
export function TodayWorkout({ onOpen }: { onOpen: () => void }) {
  ...
  return <section className="rounded-3xl border border-white/10 bg-black/30 p-5 sm:p-6" data-no-swipe>
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">Today's training</h2><button onClick={onOpen} className={button}>Edit workout plans</button></div>
    {days === undefined || plan === undefined ? <p role="status" className="mt-4 text-white/60">Loading today's training…</p> : <>
      ...
      <p className="mt-2 text-xs leading-5 text-white/55">Changing this selection keeps all logged sessions. It does not complete a workout or erase progress.</p>
      {plan?.rest && <p className="mt-5 rounded-xl border border-[rgb(var(--sw-accent-rgb)/.3)] p-4 text-sm text-white/75">Recovery is part of your plan. No workout completion is required today.</p>}
      ...
      {day && <div className="mt-5 space-y-3">
        {progress === undefined ? <p role="status">Loading this session…</p> : <>
          ...
          {exercises.length > 0 && <><button disabled={action.busy || progress?.completedWorkout || rate < 70} className={`${button} w-full`} onClick={() => void action.run(async () => { await finish({ dayId: day._id, dateKey }); toast.success("Workout saved. Your effort counts."); })}>{action.busy ? "Saving…" : progress?.completedWorkout ? "Workout saved" : rate >= 70 ? "Save completed workout" : "Check off at least 70% to finish"}</button><p className="text-xs text-white/55">Saving counts this workout once and locks its exercise checkmarks. Best-set results can still be updated.</p></>}
        </>}
      </div>}
    </>}
